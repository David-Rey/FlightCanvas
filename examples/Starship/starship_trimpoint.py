...
class Trimpoint:
    ...
    def find_yaw_ROM(self):
        ...
        fx = lambda x: self.vehicle_dynamics.dynamics(x, self.u_star)
        fu = lambda yaw: self.vehicle_dynamics.dynamics(self.x_star, self.control2deflections(0, yaw[0], 0))
    ...
    def get_control(self, state: np.ndarray) -> np.ndarray:
        ...
        #pitch_ref = np.deg2rad(np.array([5, 0, 0, 0]))  # [vx, vz, pitch_rate, pitch]
        pitch_ref = np.array([-5, 0, 0, 0])

        state_error = pitch_ref - pitch_rom #+ self.x_star_pitch
        rel_pitch_control = self.K_pitch @ state_error

        nominal_control = self.nominal_control()
        pitch_control = np.array([rel_pitch_control[0], 0, 0, 0])

        control = nominal_control + pitch_control
        return control
